...
class UpdateIssuedPermissionsTests(APITestCase):
    def setUp(self):
        self.user = get_default_user()
        self.client.force_login(self.user)
        self.script = get_default_script()
        get_default_issued(self.script, self.user)

    def test_not_authorized_user(self):
        self.client.logout()
        response = self.client.post(
            reverse(
                'scripts:script-update-issued',
...
class UpdateIssuedValidationTests(APITestCase):
    def setUp(self):
        self.user = get_default_user()
        self.client.force_login(self.user)
        self.script = get_default_script()
        self.issued = get_default_issued(self.script, self.user)
    ...
    def test_valid_license_key(self):
        ...
        for license_key in valid_license_key:
            update_issued(self.issued, license_key=license_key)
            response = self.client.post(
                reverse(
                    'scripts:script-update-issued',
                    kwargs=dict(pk=self.script.pk)
...
class UpdateIssuedTests(APITestCase):
    def setUp(self):
        self.user = get_default_user()
        self.client.force_login(self.user)
        self.script = get_default_script()
    ...
    def test_issued_record(self):
        ...
        self.assertEqual(issued['action'], 'UPDATE')
    ...
    def test_update_not_existing_license(self):
        response = self.client.post(
            reverse(
                'scripts:script-update-issued',
                kwargs=dict(pk=self.script.pk)
    ...
    def test_update_temporary_license(self):
        update_issued(self.issued, expires=date.today() + timedelta(days=10))
        response = self.client.post(
            reverse(
                'scripts:script-update-issued',
                kwargs=dict(pk=self.script.pk)
