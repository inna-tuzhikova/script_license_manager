...
class GenerateEncodedPermissionsTests(APITestCase):
    def setUp(self):
    ...
    def test_not_authorized_user(self):
        ...
        response = self.client.post(reverse(
            'scripts:script-generate-encoded',
    ...
    def test_script_not_allowing_encoded_issue_wo_force_perms(self):
        update_script(self.script, allow_issue_encoded=False)
    ...
    def test_script_not_allowing_encoded_issue_w_force_perms(self):
        give_permission_to_user(self.user, 'force_issue_encoded_script')
    ...
    def test_script_allowing_encoded_issue_wo_force_perms(self):
        response = self.client.post(
            reverse(
                'scripts:script-generate-encoded',
                kwargs=dict(pk=self.script.id)
    ...
    def test_script_allowing_encoded_issue_w_force_perms(self):
        give_permission_to_user(self.user, 'force_issue_encoded_script')
        response = self.client.post(
            reverse(
                'scripts:script-generate-encoded',
    ...
    def test_script_not_allowing_encoded_exp_issue_wo_force_perms(self):
        update_script(self.script, allow_issue_encoded_exp=False)
        response = self.client.post(
            reverse(
                'scripts:script-generate-encoded',
                kwargs=dict(pk=self.script.id)
    ...
    def test_script_not_allowing_encoded_exp_issue_w_force_perms(self):
        give_permission_to_user(self.user, 'force_issue_encoded_script')
    ...
    def test_script_allowing_encoded_exp_issue_wo_force_perms(self):
        response = self.client.post(
            reverse(
                'scripts:script-generate-encoded',
                kwargs=dict(pk=self.script.id)
    ...
    def test_script_allowing_encoded_exp_issue_w_force_perms(self):
        give_permission_to_user(self.user, 'force_issue_encoded_script')
...
class GenerateEncodedValidationTests(APITestCase):
    def setUp(self):
        self.user = get_default_user()
        self.client.force_login(self.user)
        self.script = get_default_script()
    ...
    def test_valid_params(self):
        ...
        valid_params = [
            dict(),
            dict(license_key=None, expires=None, extra_params=None),
            dict(license_key=None, expires=None),
            dict(license_key='0x12345678', expires=None),
            dict(expires=future_dt),
        ]
...
class GenerateEncodedTests(APITestCase):
    def setUp(self):
        self.user = get_default_user()
        self.client.force_login(self.user)
        self.script = get_default_script()
